"""Garde metering B-20b §7.4 — la porte coopérative (logique, testable).
...
Ajouter un site légitime = mettre à jour ``ALLOWLIST``, ce qui force la
justification en revue.

La logique vit dans le paquet (couverte par la CI) ; ``scripts/check_metering_sites.py``
n'en est qu'un point d'entrée CLI mince.
"""
...
# metering_guard.py est à <repo>/src/forgeai/models/ ; SRC = <repo>/src/forgeai.
...
# La garde elle-même est exonérée : elle DÉFINIT le marqueur (MARQUEUR ci-dessus) et le
# documente — une référence, pas une émission.
ALLOWLIST = frozenset({
    SRC / "rag" / "hardened.py",
    SRC / "models" / "probe.py",
    SRC / "models" / "metering_guard.py",
})
