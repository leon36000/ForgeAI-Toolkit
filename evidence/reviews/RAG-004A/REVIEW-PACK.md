# PACK DE REVUE (v2, ADR COMPLET) — RAG-004A DESIGN_FIRST, FAI-U-004
## v1 rejetée 3/3 pour TRONCATURE (§6.2). Version COMPLÈTE ci-dessous (§1-§9, sections 7/8/9 ajoutées).
## Évaluer le design : contrat calculable remplaçant context_used=True ; oracles par transition ; refus par défaut ;
## contrat concret RAG-005/RAG-004B (signatures, migration, rollback). Design pur, aucun code. Vérifier qu'il n'est PAS tronqué.
