...
### Tâche 1 — Sceller les régressions RED
...
- [ ] Ajouter une fixture qui appelle réellement `build_server("0.0.0.0", 0)` et utilise `127.0.0.1` seulement comme destination socket.
- [ ] Paramétrer les quatre mutations `/api/deploy`, `/api/nodes`, `/api/nodes/prepare` et `/api/models` avec un corps `{}`; un `Host: 127.0.0.1` sans Bearer doit retourner `401`.
- [ ] Ajouter le cas pur suivant:
...
- [ ] Exécuter uniquement ces tests et archiver leur échec sur la base non corrigée.
- [ ] Committer les tests RED séparément.

### Tâche 2 — Appliquer la décision fail-closed minimale
...
- [ ] Ajouter une fonction pure de classification loopback utilisant `ipaddress.ip_address(...).is_loopback` et le nom explicite `localhost`; tout nom ou wildcard non reconnu est non-loopback.
- [ ] Dans `authorize_mutation`, exiger le Bearer si un jeton est configuré ou si le bind n’est pas loopback.
- [ ] Conserver `hmac.compare_digest` pour toute comparaison d’un jeton configuré.
- [ ] Stocker le bind sur l’instance retournée par `build_server` et le lire depuis `self.server` dans `_guard_mutation`.
- [ ] Exécuter les tests RED et confirmer leur passage.
- [ ] Committer l’implémentation minimale.

### Tâche 3 — Fermer les variantes négatives

**Fichiers:** `tests/test_web_auth.py`, `src/forgeai/web/server.py`

**Produit:** refus déterministe des Bearer mal formés et ambigus.

- [ ] Tester jeton vide, préfixe partiel, casse `bearer`, suffixe, valeur incorrecte et absence.
- [ ] Envoyer deux champs `Authorization` avec `http.client`; même si le premier est valide, le résultat doit être `401`.
- [ ] Faire utiliser `get_all("Authorization")` par `_guard_mutation` et n’accepter qu’une valeur unique.
- [ ] Vérifier qu’aucune réponse, exception ou sortie de test ne contient la valeur du jeton.
- [ ] Committer la fermeture des variantes.

### Tâche 4 — Non-régression, charge et rollback

**Fichiers:** tous les fichiers de tests autorisés, puis story et registre.

**Produit:** paquet vérifiable et réversible.

- [ ] Exécuter `pytest -q tests/test_web_auth.py tests/test_web_server.py tests/test_web_nodes.py tests/test_web_deploy.py`.
- [ ] Exécuter un laboratoire HTTP réel sur `0.0.0.0`: Bearer absent/invalide `401`, Bearer valide franchit seulement la garde.
- [ ] Mesurer une boucle d’autorisation et documenter l’absence de régression significative.
- [ ] Exécuter les gates complets du dépôt, no-stub, secret scan et vérification du scope.
- [ ] Revertir les commits fonctionnels dans un worktree temporaire, confirmer l’égalité avec la base et réexécuter la baseline.
- [ ] Compléter `Registres/PATCH-WEB-001.jsonl` avec une chaîne valide.

### Tâche 5 — Revue et livraison
