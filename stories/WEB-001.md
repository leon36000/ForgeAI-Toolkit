...
- Gitleaks `8.30.1`: `426` commits du dépôt puis `10` commits du paquet, zéro fuite dans les
  deux scans redacted.
- Rollback: les `10` commits du paquet ont été inversés avec `revert --no-commit` dans un
  worktree détaché créé sous `mktemp -d`. Le tree d'index obtenu
  ...
  fichier non suivi n'existe. Le worktree temporaire, son enregistrement Git et son répertoire
  ont ensuite été supprimés explicitement.
