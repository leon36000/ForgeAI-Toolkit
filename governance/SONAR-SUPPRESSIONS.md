...
| S3776 | line | src/forgeai/renderers/compose.py:145 | équipe runtime ForgeAI | Le code de render_compose() reste inchange dans sa structure/branches (seule la resolution de project change) ; la suppression masque uniquement S3776 sur cette ligne de signature, pas d'autres regles ni d'autres sites. Risque accepte : la fonction reste difficile a maintenir en l'etat, documente ici pour une story de refactor future plutot que masque silencieusement. | Un test ne reduit pas une complexite cognitive/cyclomatique -- seul un refactor le ferait. Le comportement de render_compose() reste entierement couvert par les tests existants de tests/test_renderers.py (dont les 4 nouveaux tests de #586), qui valident deja chaque branche exercee par la fonction ; aucun test supplementaire ne compenserait la metrique de complexite elle-meme. | 2027-02-15 |
