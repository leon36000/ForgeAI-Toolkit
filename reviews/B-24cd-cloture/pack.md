# Revue de CLÔTURE — B-24c/d : garde de confinement filesystem (commit 6e9550c)

## Votre rôle est particulier — lisez ceci d'abord
Vous êtes un trio de relecteurs INÉDITS sur cette story. Aucun de vous n'a codé ni relu ce
composant. C'est délibéré : le critère d'arrêt fixé par l'architecte exige que la clôture
soit prononcée par des relecteurs HORS de la boucle qui a déjà tourné onze fois.

Onze tours de revue ont eu lieu. 24 objections, TOUTES fondées et TOUTES reproduites par
exécution avant correction. Trois relecteurs (DeepSeek, Gemini, Grok) ont épuisé leur regard
sur ce diff ; le risque d'accoutumance est réel, d'où votre intervention.

Le critère d'arrêt est explicite : la story est close si vous n'annoncez AUCUNE NOUVELLE
CLASSE de contournement. Un cas isolé dans une classe déjà traitée n'est pas une nouvelle
classe. Si vous en trouvez une, dites-le : on itère. Si vous n'en trouvez pas, dites-le
aussi clairement — c'est votre verdict qui clôt, ou non.

## Ce que fait le composant
Le produit GÉNÈRE un script de garde autonome (stdlib pure, n'importe JAMAIS le produit, il
doit survivre à sa désinstallation), branché en hook PreToolUse chez l'utilisateur. Le script
lit une charge JSON sur stdin, en extrait les chemins de fichiers visés, et REFUSE (exit 2,
journalisé) tout accès hors du workspace.

## Architecture, telle que décidée et amendée
- Racine figée par realpath À LA GÉNÉRATION (jamais dérivée du cwd à l'exécution).
- realpath des DEUX côtés ; comparaison par commonpath+normcase (jamais startswith : piège
  /repo vs /repo-evil).
- FAIL-CLOSED partout : stdin illisible, JSON invalide, toute exception interne -> REFUS.
- cwd validé EN PROPRE avant l'extraction (auto-protection puis confinement), refus journalisé.
- Extraction des chemins d'une commande Bash = UNION de deux méthodes :
    (1) shlex, qui restitue la CONCATÉNATION des parties quotées comme bash le fait ;
    (2) un découpage strict sur la classe des délimiteurs shell, appliqué DANS TOUS LES CAS.
  Le `except ValueError` de (1) ne contient que `pass` : shlex est un ENRICHISSEUR dont
  l'échec est sans conséquence, pas un primaire avec repli. C'est cette propriété qui
  interdit une porte de service — un attaquant forçant un ValueError n'obtient pas un mode
  plus permissif, puisque (2) s'applique toujours. La garantie est MONOTONE.
- Normalisation de la continuation de ligne bash avant découpage ; retrait du backslash
  d'échappement sur POSIX (conservé sur Windows où c'est le séparateur natif).
- Chemins collés à un préfixe d'option : `=` est délimiteur, et le suffixe d'un fragment
  commençant par `-` est extrait après une grappe de 1 à 3 lettres (borne MESURÉE : 0 faux
  positif à 3, deux réapparaissent à 4).
...
## Les 11 classes déjà fermées — n'en refaites pas le tour
tokens quotés · chemins Windows · relatifs sans préfixe · `.claude` parent · traversal ·
opérateurs shell collés · cwd non validé · repli permissif (2 fois) · continuation de ligne ·
backslash d'échappement · préfixes d'options (`=`, grappes) · scission par guillemets.

## Limite HORS PÉRIMÈTRE, documentée et assumée depuis l'origine
Aucune interprétation du shell : ni expansion `$VAR`, ni substitution `$(cmd)`, ni globbing.
`CIBLE=/etc/passwd; cat $CIBLE` n'est PAS arrêté — c'est le rôle d'une sandbox OS, hors
périmètre produit. Les chemins contenant un espace ne sont pas captés en entier.
Ne REJETEZ pas pour cette limite ; signalez seulement si elle est MAL BORNÉE.

## Faux positif résiduel, assumé, documenté ET testé
Un répertoire relatif dont le nom fait ≤3 lettres et collé à une grappe d'options
(`gcc -Iab/x`) est refusé à tort. Cas rare, coût accepté, verrouillé par un test de
caractérisation pour qu'une régression de la borne ne passe pas inaperçue.

## CE QUI CONSTITUERAIT UN REJECT LÉGITIME
(a) un chemin littéral hors racine qui passe ; (b) le confiné peut neutraliser sa garde ;
(c) une TROISIÈME transformation lexicale que bash applique avant le découpage en mots (les
    deux connues — continuation de ligne et échappement backslash — sont traitées, et cette
    liste a été établie par MESURE du binaire bash, pas par lecture de doc) ;
(d) une CONVENTION de collage d'un chemin à un préfixe autre que `=` ou la grappe ≤3 ;
(e) un FAUX POSITIF sur une commande de développement courante — le coût d'ergonomie compte
    autant que le confinement : une garde qui bloque `gcc -Isrc/include` sera désactivée par
    l'utilisateur, et une garde désactivée ne protège rien.

## Diff CUMULÉ vs origin/main
~~~diff
diff --git a/src/forgeai/ide/guard_fs.py b/src/forgeai/ide/guard_fs.py
new file mode 100644
index 0000000..e30c25b
--- /dev/null
...
## Preuve d'exécution CAPTURÉE (rejouée APRÈS le commit 6e9550c)
~~~
$ python3 -m pytest tests/test_guard_fs.py -q
........................s......................................s........ [ 61%]
.............................................                            [100%]
exit=0 — 116 tests, 2 skips (comportement Windows, non observable sur POSIX)

# 39 cas en subprocess reel (0=autorise, 2=refuse)
   REFUS attendus :
     2  cat /etc/passwd
     ...
     2  SCISSION /et"c/passwd"
     2  CONTINUATION ../../etc/passwd
     2  CONTINUATION .claude
     2  backslash echappe
     2  dd if=/etc/passwd
     2  awk --file=/etc/passwd
     2  tar -f/etc/passwd
     2  tar -cf/etc/passwd (grappe)
     2  tar -czf/etc/shadow (grappe 3)
     2  curl -so.claude/settings.json
     2  gcc -I/usr/include
     2  rm -rf .claude
     2  rm -rf>.claude
...
# CRITERE FIGE : refus journalise ET chaine verifiee par l'outil DU PRODUIT
   ...
   verify OFFICIEL du produit : OK /tmp/tmpi6btrwl6/reg.jsonl: 3 entrées, chaîne intègre (exit 0)

# MUTATIONS — 19 au total sur les 11 tours, substitution VERIFIEE effective a chaque fois
  commonpath->startswith · realpath->abspath · auto-protection script desactivee ·
  confinement cwd retire · auto-protection cwd retiree · decoupage reduit aux espaces ·
  backslash non retire · continuation non normalisee · '=' retire des delimiteurs ·
  borne d'index retiree · motif Windows retire (x2) · backslash du motif retire ·
  borne grappe a 1 puis a 5 · isalpha neutralise · extraction shlex retiree ·
  decoupage strict retire · repli reintroduit dans le except
  -> TOUTES ROUGE, vert au retour.
  ...
  verifie par mesure avec un symlink dans le cwd, 3 cibles, resultats identiques.

# CI : job guard-fs-multi-os ajoute (ubuntu + macos + windows, fail-fast: false)
  La CI etait 100% ubuntu ; le critere d'arret exigeait 3 OS. La garde est le seul
  composant dont le comportement depend de l'OS. Le fixture d'isolation a du etre
  rendu portable (TMPDIR + TEMP + TMP) : Windows ne lit pas TMPDIR, l'isolation serait
  tombee et 9 tests auraient echoue pour un DECOR inoperant, non pour un defaut.
