...
| Intel | `recommend_driver("intel")` et `plan_driver_op("intel", ...)` implémentées. | Qualifié sur matériel réel — déploiement e2e k3s en laboratoire : LAB-033I (evidence `reviews/LAB-033I/evidence/`) : ressource `gpu.intel.com/i915` allouée et `/dev/dri` ouvrable depuis le conteneur ; le moteur par défaut du stack (ollama) écarte l'iGPU et sert sur CPU — usage GPU réel non couvert par ce chemin, chemin Kubernetes seul prouvé. | `tests/test_intel_openvino_runtime.py`, `tests/test_gpu_reservation_vendor.py::test_render_intel_reserve_dri`, `tests/test_k3s_gpu_vendor.py::test_k3s_intel_sans_privileged`, `tests/test_k3s_gpu_vendor.py::test_k3s_intel_passthrough_dri`. |
